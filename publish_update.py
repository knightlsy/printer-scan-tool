...
def git(*args, check=True):
    shown = [ (a.replace(TOKEN, "***") if isinstance(a, str) and TOKEN in a else a) for a in args ]
    print("  $ git", " ".join(shown))
    cmd = ["git", *args]
    r = subprocess.run(cmd, capture_output=True, text=True)
    if check and r.returncode != 0:
        err = (r.stderr or r.stdout).strip()
        print("  [git stderr]", err.replace(TOKEN, "***"))
        raise RuntimeError(f"git {' '.join(shown)} 失败")
    return r
...
def push_master(version: str) -> None:
    ...
    try:
        ...
        # 分块上传经 /contents/ API 会在远端 master 直接生成 commit，导致本地落后被拒。
        # 自动 fetch + rebase 到远端之上再快进推送。
        print("  [git] push 被拒，fetch + rebase 远端后重试（分块上传会污染 master 历史）")

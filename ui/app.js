...
(function () {
  ...
  // ---------------- Tauri 桥兼容层（桌面版/Tauri 版无缝切换） ----------------
  // Tauri 端：invoke 调用后端 + event.listen 接收后端推送
  // 桌面版：window.pywebview.api.*（原样保留）
  var isTauri = !!(window.__TAURI__ && window.__TAURI__.core && window.__TAURI__.event);
  if (isTauri) {
    // 后端通过 app.emit("onX", payload) 推送，前端用 event.listen 接收
    window.__TAURI__.event.listen("onStatus", function (e) {
      var p = e.payload;
      if (typeof p === "string") window.onStatus(p, "idle");
      else if (p && p.text) window.onStatus(p.text, p.kind);
    });
    window.__TAURI__.event.listen("onProgress", function (e) {
      var p = e.payload;
      if (typeof p === "object" && p !== null) window.onProgress(p.pct, p.msg);
    });
    window.__TAURI__.event.listen("onList", function (e) { window.onList(e.payload); });
    window.__TAURI__.event.listen("onPreview", function (e) { window.onPreview(e.payload); });
    window.__TAURI__.event.listen("onConfigStatus", function (e) {
      var p = e.payload;
      if (p && p.connected !== undefined) window.onConfigStatus(p.text || "", p.connected);
    });
    window.__TAURI__.event.listen("onOverlay", function (e) {
      var p = e.payload;
      if (p && p.show !== undefined) window.onOverlay(p.show, p.cancelable);
    });
    // 监听 pick_pdf 对话框结果（Rust 通过 onPickPdf 事件推送路径）
    window.__TAURI__.event.listen("onPickPdf", function (e) {
      var p = e.payload;
      if (p && p.path) {
        state.tools.path = p.path;
        $(["toolsPdfPath"].map(function(id){ return $(id); }))[0] && ($(["toolsPdfPath"]).textContent = p.path);
        $(["toolsStatus"].map(function(id){ return $(id); }))[0] && ($(["toolsStatus"]).textContent = "");
  ...
  function apiCall(method) {
    ...
    try {
      // Tauri 桌面：walk invoke，args 展开为位置参数
      if (isTauri && window.__TAURI__ && window.__TAURI__.core) {
        // pick_pdf 通过事件返回路径，包装成 Promise
        if (method === "pick_pdf") {
          return new Promise(function (resolve) {
            ...
            try {
              window.__TAURI__.event.listen("onPickPdf", function (e) {
                unlisten();
                resolve(e.payload && e.payload.path ? e.payload.path : null);
              });
            } catch (_) { unlisten = function () {}; }
            window.__TAURI__.core.invoke("pick_pdf").catch(function () {
              unlisten();
              resolve(null);
            });
          });
        ...
        try {
          return window.__TAURI__.core.invoke(method, { args: args });
        } catch (invErr) {
          // 兼容：部分 Tauri 版本/插件把参数直接序列化为顶层字段
        }
      }
      // 原 pywebview 桌面版
